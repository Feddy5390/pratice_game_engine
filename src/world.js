export default class World {
  ...
  _componentBits = {}; // componentId -> bit
  _systems = [];
  _nextEntityId = 0;
  _recycledEntities = [];
  _queries = new Map();

  components = {}; // componentId -> store
  componentType = {};
  ...
  registerComponent(type, createStoreFn, stride) {
    const componentId = this._componentCursor++;

    this._componentBits[componentId] = 1 << componentId;
    this.componentType[type] = componentId;
  ...
  createQuery(componentIds) {
    let mask = 0;
    for (let i = 0; i < componentIds.length; i++) {
      mask |= this._componentBits[componentIds[i]];
    }
  ...
  addComponent(entityId, componentId, defaultData) {
    const bit = this._componentBits[componentId];
    const oldMask = this._masks[entityId];
    this._masks[entityId] |= bit;
    this._onMaskChanged(entityId, oldMask, this._masks[entityId]);
    this.setComponent(entityId, componentId, defaultData);
  }
  ...
  setComponent(entityId, componentId, data) {
    const { store, stride } = this.components[componentId];
